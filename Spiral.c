...
void display()
{
    ...
    float x, y;
    float angle = 0.0;
    float angle_increment = 0.1; // Change this value to adjust the spiral

    glBegin(GL_LINE_STRIP);

    for (int i = 0; i < 1000; i++) // Change this value to adjust the number of points in the spiral
    {
        x = radius * cos(angle);
        y = radius * sin(angle);
        glVertex2f(x, y);
        radius += 0.005; // Change this value to adjust the gap between each revolution
